// Régua do formulário do censo NO APP.
//
// ⚠️⚠️ CÓPIA LITERAL de `src/lib/censoForm.ts` do SISTEMA_INTEGRADO_CBRIO — byte
// a byte a partir daqui. Não é preguiça: o formulário nativo do app e o
// formulário da web precisam decidir IGUAL o que aparece, o que falta e quanto
// já foi respondido. Se as duas divergirem, o app aceita resposta que o site
// recusaria (ou barra a pessoa por uma pergunta que ela nunca viu).
//
// COMO MANTER: mudou lá, copie inteiro para cá. Não "adapte" — adaptação é
// exatamente como duas cópias começam a divergir. O servidor continua sendo a
// autoridade: ele revalida tudo no `POST /:slug/responder`.
//
// O espelho da web contra o backend é testado por `censoFormEspelho.test.ts`
// naquele repositório; aqui os casos vivem em `test/censoForm.test.ts`.
