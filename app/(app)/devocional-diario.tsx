...
export default function DevocionalDiarioScreen() {
  ...
  const [versoSelecionado, setVersoSelecionado] = useState<string | null>(null);
  ...
  const load = useCallback(
    async (comSpinner = true) => {
      ...
      try {
        const r = await semanaDevocional(membro?.membroId ?? null, ehPlanoQuarta);
        setItens(planoId ? r.itens.filter((item) => item.plano_id === planoId) : r.itens);
    ...
    [membro?.membroId, planoId, ehPlanoQuarta]
  );
  ...
  // Faixa do ciclo: planos semanais usam seg–sex; Quarta com Deus usa qui–qua.
  const seg = itens.length > 0 ? itens[0].data : null;
  const diasSemana = useMemo(() => {
    const base = new Date(`${hoje}T12:00:00`);
    const dow = base.getDay();
    const delta = ehPlanoQuarta ? (dow >= 4 ? 4 - dow : -(dow + 3)) : (dow === 0 ? -6 : 1 - dow);
    base.setDate(base.getDate() + delta);
    const labels = ehPlanoQuarta ? DIAS_QUARTA_LABEL : DIAS_SEMANA_LABEL;
  ...
  }, [hoje, checkins, itens, seg, ehPlanoQuarta]);
    ...
    <SafeAreaView style={styles.safe} edges={["left", "right"]}>
      ...
      >
        ...
        <View style={styles.header}>
          <Text style={styles.title}>{titulo || t("Devocional")}</Text>
          {streak > 0 && (
          ...
          <Pressable
            onPress={() => router.navigate("/devocional-registros")}
            hitSlop={8}
        ...
        <View style={styles.semanaCard}>
          ...
          <Text style={styles.semanaResumo}>
            {lidosSemana > 0
              ? `${lidosSemana}/${diasSemana.length} ${t("nesta semana")}`
        ...
        {loading ? (
          ...
          <View style={styles.vazio}>
            ...
            <Text style={styles.vazioTitulo}>{t("O devocional de hoje ainda não foi publicado.")}</Text>
          </View>
        ) : (
          <>
...
function makeStyles(colors: ReturnType<typeof useColors>) {
  return StyleSheet.create({
    ...
    diaLabel: { color: colors.textMuted, fontWeight: "700", fontSize: 10 },
